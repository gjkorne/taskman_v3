...
export function useTagManager({
   ...
   */
  const getSubcategoryFromTags = useCallback(
    (tagList: string[]): string | null => {
      const subcategoryTag = tagList.find((tag) =>
        tag.startsWith(SUBCATEGORY_PREFIX)
      );
      return subcategoryTag
        ? subcategoryTag.replace(SUBCATEGORY_PREFIX, '')
        : null;
    },
    []
  );
   ...
   */
  const updateSubcategoryInTags = useCallback(
    (tagList: string[], subcategory: string): string[] => {
      // Remove any existing subcategory tags
      const filteredTags = tagList.filter(
        (tag) => !tag.startsWith(SUBCATEGORY_PREFIX)
      );
