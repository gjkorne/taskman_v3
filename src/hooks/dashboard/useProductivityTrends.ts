...
export function useProductivityTrends(days: number = 14) {
  ...
  return useMemo(() => {
    ...
    const trends = {
      // Overall metrics
      totalTasksCompleted: dailyData.reduce((sum, day) => sum + day.tasksCompleted, 0),
      totalTasksCreated: dailyData.reduce((sum, day) => sum + day.tasksCreated, 0),
      totalHoursTracked: dailyData.reduce((sum, day) => sum + day.minutesTracked, 0) / 60,
    ...
    if (days >= 7) {
      ...
      const totalCompletedLast7Days = last7Days.reduce((sum, day) => sum + day.tasksCompleted, 0);
      const totalCreatedLast7Days = last7Days.reduce((sum, day) => sum + day.tasksCreated, 0);
      const totalMinutesLast7Days = last7Days.reduce((sum, day) => sum + day.minutesTracked, 0);
      
      // Tasks completed vs created rate
      trends.weeklyCompletionRate = totalCreatedLast7Days > 0 
        ? totalCompletedLast7Days / totalCreatedLast7Days 
        : 0;
      
      // Weekly average productivity
      trends.weeklyProductivity = totalMinutesLast7Days > 0 
