...
export function useTimeTrackingMetrics() {
  const { sessions, calculateTodayTimeSpent, calculateWeekTimeSpent } = useTimeSessionData();
  ...
  const metrics = useMemo(() => {
    ...
    const result = {
      ...
      taskTimeDistribution: {} as Record<string, {
        taskId: string;
        taskName: string;
        timeSpent: number;
    ...
    sessions.forEach(session => {
      ...
      if (session.task_id) {
        ...
        result.taskTimeDistribution[taskKey].formattedTime = 
          formatMillisecondsToTime(result.taskTimeDistribution[taskKey].timeSpent);
      }
...
export function useWeekTimeTracked() {
  const { timeSpentThisWeek, formattedTimeThisWeek, isLoading } = useTimeTrackingMetrics();
