...
export function useCategoryManager() {
   ...
   */
  const addSubcategory = useCallback(async (subcategoryName: string): Promise<boolean> => {
   ...
   */
  const removeSubcategory = useCallback(async (subcategoryName: string): Promise<boolean> => {
    if (!selectedCategoryId) {
      addToast('No category selected', 'error');
      return false;
    }
    ...
    return !!result;
  }, [selectedCategoryId, selectedSubcategory, getCategoryById, updateCategory, addToast]);

  /**
   * Update a subcategory name
   */
  const renameSubcategory = useCallback(async (oldName: string, newName: string): Promise<boolean> => {
    if (!selectedCategoryId) {
      addToast('No category selected', 'error');
      return false;
    }
  ...
  const createNewCategory = useCallback(async (
    ...
    const result = await createCategory(name, subcategories, color, icon, isDefault);
   ...
   */
  const makeSelectedCategoryDefault = useCallback(async (): Promise<boolean> => {
    if (!selectedCategoryId) {
      addToast('No category selected', 'error');
      return false;
    }
