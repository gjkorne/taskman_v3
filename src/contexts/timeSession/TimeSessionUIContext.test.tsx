...
function TestComponent() {
  ...
  return (
    <div>
      <span data-testid="timer-modal">
        {isTimerModalOpen ? 'open' : 'closed'}
      </span>
      <span data-testid="history-modal">
        {isHistoryModalOpen ? 'open' : 'closed'}
      </span>
      <span data-testid="selected">{selectedSessionId ?? 'none'}</span>
      <span data-testid="view">{timerDisplayMode}</span>
      <button data-testid="open-timer" onClick={openTimerModal}>
        OpenTimer
      </button>
      <button data-testid="close-timer" onClick={closeTimerModal}>
        CloseTimer
      </button>
      <button
        data-testid="open-history"
        onClick={() => openHistoryModal('sess123')}
      >
        OpenHistory
      </button>
      <button data-testid="close-history" onClick={closeHistoryModal}>
        CloseHistory
      </button>
      <button
        data-testid="set-full"
        onClick={() => setTimerDisplayMode('full')}
      >
        Full
      </button>
      <button
        data-testid="set-compact"
        onClick={() => setTimerDisplayMode('compact')}
      >
        Compact
      </button>
    </div>
