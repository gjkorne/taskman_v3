...
import {
  ...
  TIME_SESSION_QUERY_KEYS,
} from './timeSessionQueries';
...
const useTimeSessionDataHook = (): UseTimeSessionDataHookResult => {
  ...
  const { createMutation, updateMutation, deleteMutation, stopMutation } =
    useTimeSessionMutations();

  // Fetchers
  const fetchSessions = useCallback(async () => {
    await refetchSessions();
  }, [refetchSessions]);
  const getSessionsByTaskId = useCallback(
    async (taskId: string) => {
      try {
        return (
          (await queryClient.fetchQuery({
            queryKey: TIME_SESSION_QUERY_KEYS.task(taskId),
            queryFn: () => service.getSessionsByTaskId(taskId),
          })) || []
        );
      } catch (e) {
        return [];
  ...
  const getSessionsInDateRange = useCallback(
    async (startDate: Date, endDate: Date) => {
      ...
      try {
        return (
          (await queryClient.fetchQuery({
            queryKey: TIME_SESSION_QUERY_KEYS.dateRange(s, e),
            queryFn: () => service.getSessionsByDateRange(startDate, endDate),
            staleTime: 5 * 60 * 1000,
          })) || []
        );
      } catch (e) {
  ...
  const calculateTimeSpent = useCallback(
    async (taskIds?: string[], startDate?: Date, endDate?: Date) => {
      try {
        return await service.calculateTimeSpent(taskIds, startDate, endDate);
      } catch {
        return 0;
      }
    },
    ...
    try {
      return (
        (await queryClient.fetchQuery({
          ...
          queryFn: () => calculateTimeSpent(undefined, today, tomorrow),
          staleTime: 5 * 60 * 1000,
        })) || 0
      );
    } catch {
    ...
    try {
      return (
        (await queryClient.fetchQuery({
          ...
          queryFn: () => calculateTimeSpent(undefined, startOfWeek, endOfWeek),
          staleTime: 5 * 60 * 1000,
        })) || 0
      );
    } catch {
  ...
  return {
    ...
    mutations: {
      createSession: useCallback(
        async (taskId: string) => {
          try {
            return await createMutation.mutateAsync(taskId);
          } catch {
            return null;
          }
        },
      ...
      updateSession: useCallback(
        async (id: string, data: Partial<TimeSession>) => {
          try {
      ...
      stopSession: useCallback(
        async (id: string) => {
          try {
            return await stopMutation.mutateAsync(id);
          } catch {
            return null;
          }
        },
