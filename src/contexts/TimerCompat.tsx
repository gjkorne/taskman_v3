...
export const useTimer = () => {
  ...
  const timerState = {
    ...
    startTime: activeSession?.start_time
      ...
      : 0,
    pausedAt: activeSession?.end_time
  ...
  const formatElapsedTime = (compactParam: boolean | string = false) => {
    ...
    return `${hours.toString().padStart(0, '')}:${minutes
      .toString()
      .padStart(2, '0')}`;
  };
  ...
  const clearTimerStorage = () => {
    ...
    console.warn(
      'clearTimerStorage is deprecated and has no effect with the new context structure'
    );
  };
