...
export const { Provider: TaskUIProvider, useUIContext: useTaskUI } = createUIContext({
  displayName: 'Task',
  initialState: {
  ...
  actions: (_state, setState) => ({
    openEditModal: (taskId: string) => setState(s => ({ ...s, editTaskId: taskId, isEditModalOpen: true })),
    closeEditModal: () => {
      setState(s => ({ ...s, isEditModalOpen: false }));
      setTimeout(() => setState(s => ({ ...s, editTaskId: null })), 300);
    },
    openDeleteModal: (taskId: string) => setState(s => ({ ...s, taskToDelete: taskId, isDeleteModalOpen: true })),
    closeDeleteModal: () => {
