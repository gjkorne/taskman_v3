        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-1 sm:space-x-2">
            <button
              ...
              aria-label={
                viewMode === 'month' ? 'Previous month' : 'Previous day'
              }
            >
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      days.push(
        <div
          key={i}
          className="font-medium text-center text-gray-600 text-xs py-2"
        >
          {format(addDays(start, i), 'EEEEE')}
          <span className="hidden sm:inline">
            {format(addDays(start, i), 'EEE').slice(1)}
          </span>
        </div>
      );
    }
          ...
          >
            <div className="flex justify-between items-start">
              <span
                ...
                ${
                  isTodayDate
                    ? 'font-bold h-5 w-5 rounded-full bg-blue-500 text-white'
              ...
              {tasksForDay.length > 0 ? (
                tasksForDay.map((task, index) =>
                  ...
                  index < 2 || window.innerWidth > 640 ? (
                    <div
                      key={task.id}
                      className={`text-xs p-1 rounded truncate ${getPriorityClass(
                        task.priority
                      )}`}
                      title={task.title}
                    >
                    ...
                    <div
                      key="more"
                      className="text-xs p-1 text-gray-500 text-center"
                    >
                ...
                <div className="hidden sm:block text-xs text-gray-400 italic p-1">
                  No tasks
                </div>
              )}
            </div>
