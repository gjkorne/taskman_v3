...
import { BarChart as BarChartIcon, PieChart as PieChartIcon, List } from 'lucide-react';
...
export function TimeReportView({ data, isLoading = false }: TimeReportViewProps) {
  ...
  console.log('Total time seconds:', totalTimeSeconds, 'formatted as:', formattedTotalTime);

  // Calculate percentages for each item
  const dataWithPercentages = data.map(item => ({
    ...item,
    percentage: totalTimeSeconds > 0 ? (item.value / totalTimeSeconds) * 100 : 0
  }));
  ...
  if (data.length === 0) {
    return (
      <div className="text-center py-10">
        <p className="text-lg text-gray-500">No time data available for the selected period.</p>
        <p className="text-sm text-gray-400 mt-2">Try changing your filters or logging some time sessions.</p>
      </div>
    );
  }
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-right mb-4">
          ...
          <div className="text-3xl font-bold text-indigo-600">{formattedTotalTime}</div>
        </div>
        <div className="flex space-x-1 border rounded-md overflow-hidden">
          ...
          <button
            ...
            className={`p-2 ${viewType === 'pie' ? 'bg-indigo-100 text-indigo-700' : 'bg-white text-gray-700'}`}
            title="Pie Chart"
          >
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {dataWithPercentages.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.formattedValue}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.percentage.toFixed(1)}%</td>
...
function BarChart({ data }: { data: Array<TimeReportItem & { percentage: number }> }) {
      ...
      tooltip: {
        callbacks: {
          label: function(context: any) {
            return `${context.parsed.y} hours (${chartData[context.dataIndex].formattedValue})`;
          }
        }
      }
    },
...
function PieChart({ data }: { data: Array<TimeReportItem & { percentage: number }> }) {
    ...
    datasets: [
      {
        ...
        backgroundColor: generateColors(visibleData.length + (hasOther ? 1 : 0)),
        borderWidth: 1,
      },
    ],
  };
