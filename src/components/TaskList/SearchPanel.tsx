  ...
  return (
    <div className="flex flex-wrap gap-2 items-center relative">
      ...
      <button
        ...
        className={`absolute right-0 top-0 p-2 rounded-md bg-gray-100 hover:bg-gray-200 transition-colors md:hidden z-10 ${isRefreshing ? 'animate-spin' : ''}`}
      ...
      <div className="flex-grow flex items-center min-w-[260px] max-w-md">
        ...
        <button
          onClick={() => refreshTasks()}
          className={`ml-2 p-2 rounded-md bg-gray-100 hover:bg-gray-200 transition-colors hidden md:block ${isRefreshing ? 'animate-spin' : ''}`}
          disabled={isRefreshing}
          aria-label="Refresh tasks"
        >
          <RefreshCw className="h-4 w-4 text-gray-600" />
