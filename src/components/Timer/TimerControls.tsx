  ...
  const handleStartTimer = async () => {
    ...
    if (timerState.status !== 'idle' && timerState.taskId !== taskId) {
      ...
      if (settings.allowTaskSwitching) {
        if (DEBUG_TIMER)
          console.log(
            'TimerControls: Switching from task',
            currentTaskId,
            'to',
            taskId
          );
        ...
        addToast(
          'Previous timer has been stopped and a new one started.',
          'info'
        );
        notifyStateChange();
      } else {
    ...
    return (
      ...
      >
        ...
        <div className="flex justify-between text-xs px-1 font-medium">
          <span className="text-green-700">Task timing active</span>
  ...
  if (isPaused) {
    return (
      ...
      >
        ...
        <div className="flex justify-between text-xs px-1 font-medium">
          <span className="text-amber-700">Timer paused</span>
          <span className="font-mono">{formatElapsedTime()} elapsed</span>
        </div>
      </div>
    );
