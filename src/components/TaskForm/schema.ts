...
export const taskFormSchema = z.object({
  ...
  estimatedMinutes: z.string().optional()
    .refine(val => val === undefined || val === '' || /^\d{1,2}$/.test(val), 'Minutes must be a number between 0-59')
    .refine(val => val === undefined || val === '' || (parseInt(val) >= 0 && parseInt(val) <= 59), 'Minutes must be between 0-59'),
  ...
  context: z.array(z.enum(['work', 'personal', 'home', 'errands'])).default([]),
});
