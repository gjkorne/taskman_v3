...
import { IUserService, UserProfile, UserServiceEvents } from '../interfaces/IUserService';
 ...
 */
export class UserService extends BaseService<UserServiceEvents> implements IUserService {
   ...
   */
  async getUserProfile(): Promise<{ profile: UserProfile | null; error: ServiceError | null }> {
    try {
   ...
   */
  async updateUserProfile(data: Partial<UserProfile>): Promise<{ profile: UserProfile | null; error: ServiceError | null }> {
    try {
   ...
   */
  async hasCompletedOnboarding(): Promise<{ completed: boolean; error: ServiceError | null }> {
    try {
      ...
      const serviceError = this.processError(error, 'user.onboarding_check_error');
      this.emit('error', serviceError);
   ...
   */
  async completeOnboarding(): Promise<{ success: boolean; error: ServiceError | null }> {
    try {
      ...
      const serviceError = this.processError(error, 'user.onboarding_complete_error');
      this.emit('error', serviceError);
  ...
  async getUserActivityStats(): Promise<{ 
    ...
    try {
      ...
      const timeLogged = timeSessions?.reduce((total, session) => {
   ...
   */
  async updateLastActive(): Promise<{ success: boolean; error: ServiceError | null }> {
    try {
