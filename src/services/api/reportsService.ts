...
import { formatDurationHumanReadable, parseDurationToSeconds } from '../../utils/timeUtils';
...
class ReportsService {
  async getTimeReport(filters: ReportFilter): Promise<TimeReportItem[]> {
    const { startDate, endDate, taskIds, categoryNames, groupBy = 'task' } = filters;
    ...
    console.log('Fetching time sessions with filters:', JSON.stringify(filters, null, 2));
    ...
    const filteredData = data.filter(session => {
      ...
      if (categoryNames?.length && !categoryNames.includes(session.tasks.category_name)) {
        return false;
      }
    ...
    console.log('Sample session data:', filteredData.length > 0 ? JSON.stringify(filteredData[0], null, 2) : 'No sessions');
    ...
    switch (groupBy) {
      case 'task':
        ...
        filteredData.forEach(session => {
          ...
          console.log(`Processing session ${session.id} for task "${taskTitle}"`);
          
          // First try to use the duration field if it exists
          if (session.duration) {
            console.log(`Session ${session.id} has duration: "${session.duration}"`);
          ...
          // If no duration but we have start and end time, calculate it
            ...
            try {
              console.log(`Session ${session.id} calculating from start/end times:`, session.start_time, session.end_time);
              ...
              console.error('Error calculating duration for session:', session.id, err);
            }
          } else {
          ...
          if (durationSeconds > 0) {
            console.log(`Adding ${durationSeconds}s for session ${session.id} (${taskTitle})`);
            groupedData[taskId].value += durationSeconds;
            groupedData[taskId].formattedValue = formatDurationHumanReadable(groupedData[taskId].value);
            groupedData[taskId].count = (groupedData[taskId].count || 0) + 1;
            console.log(`Updated total for ${taskTitle}: ${groupedData[taskId].value}s (${groupedData[taskId].formattedValue})`);
          } else {
      ...
      case 'category':
        ...
        filteredData.forEach(session => {
          ...
          console.log(`Processing session ${session.id} for category "${categoryName}"`);
          ...
          // Only add to the total if we have a valid duration
          if (durationSeconds > 0) {
            console.log(`Adding ${durationSeconds}s for session ${session.id} (${categoryName})`);
            groupedData[categoryName].value += durationSeconds;
            groupedData[categoryName].formattedValue = formatDurationHumanReadable(groupedData[categoryName].value);
            groupedData[categoryName].count = (groupedData[categoryName].count || 0) + 1;
            console.log(`Updated total for ${categoryName}: ${groupedData[categoryName].value}s (${groupedData[categoryName].formattedValue})`);
          } else {
            console.warn(`Session ${session.id} has invalid or zero duration`);
          }
        });
      ...
      case 'day':
        ...
        filteredData.forEach(session => {
          ...
          const dayDisplay = new Date(dayStr).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
          ...
          console.log(`Processing session ${session.id} for day "${dayDisplay}"`);
          ...
          // Only add to the total if we have a valid duration
          if (durationSeconds > 0) {
            console.log(`Adding ${durationSeconds}s for session ${session.id} (${dayDisplay})`);
            groupedData[dayStr].value += durationSeconds;
            groupedData[dayStr].formattedValue = formatDurationHumanReadable(groupedData[dayStr].value);
            groupedData[dayStr].count = (groupedData[dayStr].count || 0) + 1;
            console.log(`Updated total for ${dayDisplay}: ${groupedData[dayStr].value}s (${groupedData[dayStr].formattedValue})`);
          } else {
            console.warn(`Session ${session.id} has invalid or zero duration`);
          }
        });
      ...
      case 'week':
        ...
        filteredData.forEach(session => {
          ...
          console.log(`Processing session ${session.id} for week "${weekLabel}"`);
          ...
          // Only add to the total if we have a valid duration
          if (durationSeconds > 0) {
            console.log(`Adding ${durationSeconds}s for session ${session.id} (${weekLabel})`);
            groupedData[weekKey].value += durationSeconds;
            groupedData[weekKey].formattedValue = formatDurationHumanReadable(groupedData[weekKey].value);
            groupedData[weekKey].count = (groupedData[weekKey].count || 0) + 1;
            console.log(`Updated total for ${weekLabel}: ${groupedData[weekKey].value}s (${groupedData[weekKey].formattedValue})`);
          } else {
            console.warn(`Session ${session.id} has invalid or zero duration`);
          }
        });
  ...
  async getProductivityReport(filters: ReportFilter): Promise<any[]> {
    ...
    data.forEach(task => {
      ...
      const dayLabel = updatedDate.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' });
      ...
      groupedByDay[dayKey].formattedValue = `${groupedByDay[dayKey].value} tasks`;
    });
  ...
  async getTaskCompletionReport(filters: ReportFilter): Promise<any[]> {
    ...
    return Object.values(groupedByCategory).sort((a: any, b: any) => b.value - a.value);
  }
}
