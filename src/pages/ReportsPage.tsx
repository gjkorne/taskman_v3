...
export function ReportsPage() {
    ...
    async function fetchCategories() {
      try {
        ...
        const uniqueCategories = Array.from(
          new Set(
            data.map((task) => task.category_name).filter(Boolean) as string[]
          )
        );
    ...
    try {
      switch (activeReport) {
        case 'time':
          ...
          break;
        case 'productivity':
          const productivityData = await reportsService.getProductivityReport(
            filters
          );
          setProductivityData(productivityData);
          break;
        case 'completion':
          const completionData = await reportsService.getTaskCompletionReport(
            filters
          );
  ...
  const debugTimeSessions = async () => {
    ...
    try {
      ...
      const sessionsToFix = data.filter(
        (session) => session.start_time && session.end_time && !session.duration
      );
      ...
      if (sessionsToFix.length > 0) {
        ...
        fixResults =
          'No sessions with missing durations found. Checking for other issues...\n';
      }
...
${sessionSummary}
  ...
  const forceRecalculateTimeReport = async () => {
    ...
    try {
      ...
      const filteredSessions = data.filter((session) => {
        ...
        return (
          sessionDate >= filters.startDate && sessionDate <= filters.endDate
        );
      });
      ...
      filteredSessions.forEach((session) => {
        ...
        if (session.duration) {
          ...
          console.log(
            `Session ${session.id} - "${taskTitle}" - duration: "${session.duration}" → ${durationSeconds}s`
          );
          ...
          console.log(
            `Session ${session.id} - "${taskTitle}" - calculated: ${durationSeconds}s from times`
          );
        }

        if (durationSeconds > 0) {
          taskGroups[taskId].value += durationSeconds;
          taskGroups[taskId].formattedValue = formatDurationHumanReadable(
            taskGroups[taskId].value
          );
          taskGroups[taskId].count++;
          totalDuration += durationSeconds;

          console.log(
            `Updated "${taskTitle}" total: ${taskGroups[taskId].value}s (${taskGroups[taskId].formattedValue})`
          );
        }
      });
      ...
      console.log(
        `Recalculated time report: ${timeReportItems.length} items, total duration: ${totalDuration}s`
      );
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {isDebugMode && (
        <div
          className="mb-6 bg-gray-50 border p-4 rounded-lg overflow-auto text-sm font-mono"
