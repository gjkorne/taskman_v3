...
export function parseDurationToSeconds(durationStr: string | null): number {
  ...
  if (secondsMatch) {
    ...
    if (!isNaN(seconds)) {
      if (DEBUG_DURATION_PARSING)
        console.log(
          `Parsed duration "${originalInput}" as ${seconds} seconds (direct seconds format)`
        );
      return seconds;
    }
  }
  ...
  const timeMatch = durationStr.match(
    /^(\d{1,2}):(\d{1,2}):(\d{1,2})(?:\.\d+)?$/
  );
  if (timeMatch) {
    ...
    if (DEBUG_DURATION_PARSING)
      console.log(
        `Parsed duration "${originalInput}" as ${result} seconds (HH:MM:SS format)`
      );
    return result;
  }

  // Try to parse PostgreSQL ISO 8601 format: P0Y0M0DT0H30M0S
  const isoMatch = durationStr.match(
  ...
  if (isoMatch) {
    ...
    if (DEBUG_DURATION_PARSING)
      console.log(
        `Parsed duration "${originalInput}" as ${totalSeconds} seconds (ISO format)`
      );
    return totalSeconds;
  }
  ...
  if (matched) {
    if (DEBUG_DURATION_PARSING)
      console.log(
        `Parsed duration "${originalInput}" as ${totalSeconds} seconds (verbose format)`
      );
    return totalSeconds;
  }

  // Try to parse "X days HH:MM:SS" format
  const daysTimeMatch = durationStr.match(
  ...
  if (daysTimeMatch) {
    ...
    if (DEBUG_DURATION_PARSING)
      console.log(
        `Parsed duration "${originalInput}" as ${totalSeconds} seconds (days + HH:MM:SS format)`
      );
    return totalSeconds;
  }
  ...
  if (pgIntervalMatch) {
    ...
    if (DEBUG_DURATION_PARSING)
      console.log(
        `Parsed duration "${originalInput}" as ${result} seconds (PostgreSQL interval notation)`
      );
    return result;
  }

  // Try to parse microseconds format (e.g. "15:00:00.123456")
  const microsecondsMatch = durationStr.match(
    /^(\d{2}):(\d{2}):(\d{2})\.(\d{1,6})$/
  );
  if (microsecondsMatch) {
    ...
    if (DEBUG_DURATION_PARSING)
      console.log(
        `Parsed duration "${originalInput}" as ${result} seconds (microseconds format)`
      );
    return result;
  }
...
export function formatDurationHumanReadable(
  ...
  if (typeof duration === 'number') {
    ...
    if (DEBUG_DURATION_PARSING)
      console.log(
        `formatDurationHumanReadable: using number input directly: ${totalSeconds}s`
      );
  } else {
    totalSeconds = parseDurationToSeconds(duration);
    if (DEBUG_DURATION_PARSING)
      console.log(
        `formatDurationHumanReadable: parsed from string: "${duration}" → ${totalSeconds}s`
      );
  }
...
export function calculateTotalDuration(
  sessions: Array<{ duration: string | null }>
): string {
  let totalSeconds = 0;
 ...
 */
export function calculateDurationBetweenOld(
  startTime: string,
  endTime: string
): number {
  try {
