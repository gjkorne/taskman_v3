 ...
 * This will identify categories with z_ prefix and their non-prefixed counterparts
 * and merge them by transferring any tasks from the z_ version to the non-z_ version
 * then deleting the z_ version
 */
...
/**
 * Check for duplicate categories without making any changes
 * This is useful for diagnosing the issue before running the cleanup
 */
export async function checkDuplicateCategories() {
...
// Create a namespace for our category utilities
const categoryUtils = {
  cleanup: cleanupDuplicateCategories,
  check: checkDuplicateCategories
};

// Make sure our utilities are available globally
if (typeof window !== 'undefined') {
  (window as any).categoryUtils = categoryUtils;
  (window as any).cleanupCategories = cleanupDuplicateCategories;
}

console.log('Category cleanup utility loaded. Use window.categoryUtils.check() to check for duplicates or window.categoryUtils.cleanup() to run the cleanup.');
