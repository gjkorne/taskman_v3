import {
  ...
  CreateTaskApiDto,
  UpdateTaskApiDto,
} from '../../types/api/taskDto';
import {
  TaskModel,
  NewTaskInput,
  TaskUpdateInput,
} from '../../types/models/TaskModel';
 ...
 */
export class TaskTransformer extends BaseDataTransformer<
  TaskApiDto,
  TaskModel
> {
  /**
  ...
  toApi(model: TaskModel): TaskApiDto {
    return {
      ...
      estimated_time: this.minutesToIntervalString(
        model.estimatedTimeMinutes ?? null
      ),
      actual_time: this.minutesToIntervalString(
  ...
  updateTaskToApiDto(input: TaskUpdateInput): UpdateTaskApiDto {
    ...
    if (input.estimatedTimeMinutes !== undefined) {
      result.estimated_time = this.minutesToIntervalString(
        input.estimatedTimeMinutes
      );
    }
    if (input.actualTimeMinutes !== undefined) {
      result.actual_time = this.minutesToIntervalString(
    ...
    if (input.categoryName !== undefined)
      result.category_name = input.categoryName;
  ...
  toUpdateTaskInput(model: Partial<TaskModel>): UpdateTaskApiDto {
    ...
    if (model.dueDate !== undefined)
      updateDto.due_date = model.dueDate ? model.dueDate.toISOString() : null;
    if (model.estimatedTimeMinutes !== undefined)
      updateDto.estimated_time = this.minutesToIntervalString(
        model.estimatedTimeMinutes ?? null
      );
    if (model.actualTimeMinutes !== undefined)
      updateDto.actual_time = this.minutesToIntervalString(
        model.actualTimeMinutes ?? null
      );
    ...
    if (model.categoryName !== undefined)
      updateDto.category_name = model.categoryName;

    // Add NLP fields if they're present in the model
    if (model.confidenceScore !== undefined)
      updateDto.confidence_score = model.confidenceScore;
    if (model.processingMetadata !== undefined)
      updateDto.processing_metadata = model.processingMetadata;
    if (model.nlpTokens !== undefined) updateDto.nlp_tokens = model.nlpTokens;
    if (model.extractedEntities !== undefined)
      updateDto.extracted_entities = model.extractedEntities;
    if (model.embeddingData !== undefined)
      updateDto.embedding_data = model.embeddingData;
    if (model.notes !== undefined) updateDto.notes = model.notes;
    if (model.checklistItems !== undefined)
      updateDto.checklist_items = model.checklistItems;
