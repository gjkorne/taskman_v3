...
export const RepositoryProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  return (
    <RepositoryContext.Provider
      value={{ categoryRepository, userPreferencesRepository }}
    >
      {children}
    </RepositoryContext.Provider>
  );
...
export const CategoryProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const toggleQuickTaskCategory = useCallback(
    async (categoryId: string) => {
      ...
      await updatePreferencesMutation.mutateAsync({
        quick_task_categories: quickTaskCategories,
      });
    },
  ...
  const setDefaultCategory = useCallback(
    async (categoryId: string) => {
      await updatePreferencesMutation.mutateAsync({
        default_quick_task_category: categoryId,
      });
    },
  ...
  return (
    <AuthProvider>
      <RepositoryProvider>
        <QueryProvider>
          <SettingsProvider>
            <CategoryProvider>{/* Rest of the application */}</CategoryProvider>
          </SettingsProvider>
        </QueryProvider>
      </RepositoryProvider>
    </AuthProvider>
...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <button
        onClick={() =>
          createCategory({ name: 'New Category', color: '#ff0000' })
        }
      >
        Add Category
      </button>
